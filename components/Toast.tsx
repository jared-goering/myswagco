  ...
  return (
    <AnimatePresence>
      {isVisible && (
        ...
        >
          <div className={`flex items-center gap-3 px-4 py-3 rounded-lg border-2 ${current.bgColor} ${current.borderColor} shadow-xl backdrop-blur-md`}>
            <div className={current.iconColor}>
              {current.icon}
            </div>
            <p className={`flex-1 font-medium ${current.textColor}`}>
              {message}
            </p>
            <button
              onClick={onClose}
